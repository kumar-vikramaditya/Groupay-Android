			...
			public void onClick(View v) {
				...
				//Log.e("groupMemberRequestName",outputStrArr.toString());
	             try {
		             ...
		             params.add(new BasicNameValuePair("groupName", et_grpname.getText().toString()));
		             params.add(new BasicNameValuePair("groupAdminEmail", user.getString("email")));
					 params.add(new BasicNameValuePair("groupAdminName", user.getString("firstName")+" "+user.getString("lastName")));
					 for(int j=0;j<outputStrArrEmail.length;j++){
						// Log.e("groupMemberRequestNameinloopss",outputStrArr[j]);
						 params.add(new BasicNameValuePair("groupMemberRequestEmail", outputStrArrEmail[j]));
			             params.add(new BasicNameValuePair("groupMemberRequestName", outputStrArr[j]));
					 }
					 
		             JSONObject json = sr.getJSON(AppSettings.SERVER_IP+"/createGroup",params);
		            if(json != null){
