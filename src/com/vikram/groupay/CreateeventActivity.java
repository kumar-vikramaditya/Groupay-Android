...
public class CreateeventActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			values_group_name_list = new String[user.getJSONArray("groups")
					.length()];
			Log.e("user.getJSONArra", new Integer(user.getJSONArray("groups")
					.length()).toString());
			for (int i = 0; i < user.getJSONArray("groups").length(); i++) {
				ServerRequest sr = new ServerRequest();
				params = new ArrayList<NameValuePair>();
				params.add(new BasicNameValuePair("groupToken", user
						.getJSONArray("groups").getString(i)));
				...
				if (json != null) {
					...
					eventMembersName.add(i,
							json.getJSONArray("groupMembersName"));
		...
		createEvent.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				try {
					ServerRequest sr = new ServerRequest();
					params = new ArrayList<NameValuePair>();
					params.add(new BasicNameValuePair("eventName", et_eventname
							.getText().toString()));
					params.add(new BasicNameValuePair("eventDescription",
							et_eventdesc.getText().toString()));
					params.add(new BasicNameValuePair("venue", et_eventvenue
							.getText().toString()));
					params.add(new BasicNameValuePair("eventDate", month + "/"
							+ day + "/" + year));
					params.add(new BasicNameValuePair("eventTime", hour + ":"
							+ minute));

					params.add(new BasicNameValuePair("eventAdminEmail", user
							.getString("email")));
					params.add(new BasicNameValuePair("eventAdminName", user
							.getString("userName")));
					params.add(new BasicNameValuePair("groupName",
							values_group_name_list[globalPosition]));

					params.add(new BasicNameValuePair("eventMembers", user
							.getString("email")));

					for (int i = 0; i < eventMembers.get(globalPosition)
							.length(); i++) {
						if(!user.getString("email").equals(eventMembersName.get(globalPosition).getString(
								i))){
							Log.e("InFinalFor",user.getString("email")+"::::>>"+eventMembers.get(globalPosition).getString(i));
							params.add(new BasicNameValuePair("eventRequests",
									eventMembers.get(globalPosition).getString(i)));
							params.add(new BasicNameValuePair("eventRequestsName",
									eventMembersName.get(globalPosition).getString(
											i)));
						}
					...
					if (json != null) {
						...
						Toast.makeText(getApplication(),
								"Event created successfully", Toast.LENGTH_LONG)
								.show();
						finish();
					}
				} catch (JSONException e) {
		...
		ArrayAdapter<String> adapter_group_list = new ArrayAdapter<String>(
				this, android.R.layout.simple_list_item_1,
				values_group_name_list);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.action_logout:
			...
			// edit.commit();
			Intent mainactivity = new Intent(CreateeventActivity.this,
					MainActivity.class);
